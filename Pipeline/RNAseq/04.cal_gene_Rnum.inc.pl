##usally use the output file of 02 (reads or pair-end reads coverage file) or 01(reads map to genome or junction), 
##map reads to CDS region, calculate reads number supporting gene expression 
##map reads to UTR, UTRextension, intron region.. count numbers, output these info for furture use

#07/06/2011 add function: can use transcript rather than CDS to calculate gene read number by specify $countRnum_inRegion for refseq transcript
#07/08/2011 add code: if region>2*reads length, require reads must be inside the feature bundary
#1/25/2012  by setting $if_reverse_read_strand, can reverse read strand
#03/29/2012 add function to output read number in different region (@comprehensive_out_regions) of transcript by specify $countRnum_inRegion="comprehensive" 
#03/17/2014 add code to allow redefine refseq 3'end by 3'READS defined pA sites (by setting $gene3end_redefine_f)
#05/09/2018 fixed a bug that junction read mapped to "intron|UTR3e|UTR5e" was not outputted by previous script. 
		...
		foreach $reg_type(("CDS","3UTR","5UTR","exon","UTR3e","UTR5e")){
			foreach my $block_num(($posfrom_block_num..$posto_block_num)){
				foreach my $key2 ( keys %{$chro2reginfo_hash{"$reg_type.$chromosome:$block_num"}} ){
					...
					$treated_ids{$rowi}=1; #debugged: added 2021/07/25 to make sure one read only count once for one refseq ID
