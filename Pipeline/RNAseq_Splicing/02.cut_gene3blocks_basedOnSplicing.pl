##cut a gene to blocks based on splicing information from Deep sequencing data.
##7/27/2015 add more annotations for alternative splicing
##1/23/2017 add filtering method for removing junctions with low "relative number" of reads compared to average junction supporting read number in the gene (can set a low ratio cutoff $novelJunc_SuppRelReadMin eg. 0.03)
##1/23/2017 add total junction read number for alternative splicing annotations (use %junc2totalReadNum_h, can be used to filter alternative events which has lower read number)
##4/3/2017 also output a file with annotations for each site (splice sites, etc) of a gene to split a gene to blocks
